...
class AdminServlet(
  ...
  before() {
    ...
    log.reportAdminHttpRequest(requestLog)
  ...
  private def parseTaskKeyFromParams(
    ...
    parseTaskKey match {
      case Success(key) => successAction(key)
  ...
  private def logResponse(actionResult: ActionResult): ActionResult = {
    lazy val responseLog = s"Scheduler API: Returning ${actionResult.status}"
    log.reportAdminHttpResponse(responseLog)
    actionResult
  }
}
